...
class _ResetPasswordScreenState extends ConsumerState<ResetPasswordScreen> {
  ...
  Future<bool> requestAuthCode(WidgetRef ref, String phoneNumber) async {
    ...
    try {
      final response = await HttpsService.postRequest('/StoreAdmin/generate-verification-code', jsonBody);
      if(response.statusCode == 200){
        final responseBody = json.decode(utf8.decode(response.bodyBytes));
        if(responseBody['status'] == '200'){
          await showAlertDialog(ref.context, "인증번호 요청", "성공! SMS를 확인해주세요", null);
          return true;
        }else{
      ...
      await showAlertDialog(ref.context, "비밀번호 변경", "인증코드 요청오류\n에러내용$error", null);
      return false;
    }finally{
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      if(true == await ref.read(loginProvider.notifier).resetPassword(ref, adminPhoneNumber, newPassword, authCode)){
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 15),
                    ElevatedButton(
                      onPressed: () => {ref.read(requestAuthCodeButtonStateProvider.notifier).state ? requestAuthCode(ref, _phoneNumberController.text) : null},
                      style: ElevatedButton.styleFrom(
                        backgroundColor: ref.watch(requestAuthCodeButtonStateProvider.notifier).state ? Color(0xFF77AAD8) : Colors.grey,
