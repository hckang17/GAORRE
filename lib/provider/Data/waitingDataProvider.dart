...
class WaitingDataNotifier extends StateNotifier<WaitingData?> {
  ...
  */
  ...
  Future<bool> requestUserCall(WidgetRef ref, String phoneNumber,
      int waitingNumber, int storeCode, int minutesToAdd) async {
    print('고객 호출 - waitingNumber $waitingNumber');
    String storeName =
        ref.read(storeDataProvider.notifier).getStoreData()!.storeName;
    ...
    final response =
        await HttpsService.postRequest('/StoreAdmin/userCall', jsonBody);
            ...
            storeCode) //고객호출 성공시! 이때, storeCode가 같은지로 확인함.
        {
          ...
          currentState!.teamInfoList.forEach((waitingTeam) {
            if (waitingTeam.waitingNumber == callGuestResponse.waitingTeam) {
              waitingTeam.entryTime =
                  DateTime.parse(callGuestResponse.entryTime);
            }
          });
          ...
          print(SMScontent);
          bool result = await SendSMSService.requestSendSMS(
              ref.context, phoneNumber, "[웨이팅 호출]", SMScontent);
          if (result)
            await showAlertDialog(ref.context, "웨이팅 호출 SMS 전송", "성공!", null);
          return true;
        } else {
          // 고객호출 실패
          print('고객호출 실패');
          await showAlertDialog(
              ref.context, '$waitingNumber번 고객 호출', '고객호출 실패', null);
          return false;
        }
      } catch (error) {
  ...
  Future<void> requestUserDelete(
      BuildContext context, int storeCode, int noShowWaitingNumber) async {
    print('웨이팅유저 삭제 요청');
    final jsonBody = json.encode({
      "storeCode": storeCode,
  ...
  Future<bool> addWaitingTeam(BuildContext context, LoginData loginData,
      String phoneNumber, int personCount) async {
    print('수동 웨이팅 팀 추가 요청 발생 [waitingProvider - addWaitingTeam]');
    //자 여기에 기존 state에 동일한 phoneNumber의 웨이팅 유저가 있는지?
    int existingTeamIndex = state!.teamInfoList
    ...
    try {
      _client?.send(
          destination:
              '/app/admin/waiting/make/${loginData.storeCode}/$phoneNumber',
          body: jsonBody);
  ...
  Future<void> subscribeToWaitingData(int storeCode) async {
    ...
    if (_client == null || !_client!.connected) {
      print(
          'STOMP가 연결되어 있지 않습니다. Subscription aborted. [waitingProvider - subscribeToWaitingData]');
      await Future.delayed(Duration(seconds: 5));
      print(
          'WaitingData 구독을 재시도 합니다... [waitingProvider - subscribeToWaitingData]');
  ...
  Future<bool> confirmEnterance(
    ...
    try {
      final response =
          await HttpsService.postRequest('/StoreAdmin/entering', jsonBody);
      if (response.statusCode == 200) {
        final responseBody = json.decode(utf8.decode(response.bodyBytes));
...
String extractEntryTimeInMinutes(String message) {
  DateTime parsedTime = DateTime.parse(message);
  String formattedTime =
      '${parsedTime.hour.toString().padLeft(2, '0')}시${parsedTime.minute.toString().padLeft(2, '0')}분';
  return formattedTime;
}

String SMScontentString(
    int waitingNumber, String storeName, String deadlineTime) {
  String result =
      "안녕하세요~! $waitingNumber번 고객님! $storeName에 입장하실 시간이에요! $deadlineTime까지 내점해주세요~!";
  return result;
}
